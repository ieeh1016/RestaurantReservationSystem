...
<script type="text/javascript">
	function changeForm() {
		location.href = "ModifyReservation.jsp";
	}
</script>
</head>
<body>
					...
					<table class="table">
						...
						<tbody>
							...
							try {
								...
								"select r.oid, c.name, c.phoneNumber, r.covers, r.date, r.time, r.table_id, m.menuName, c.oid, c.point from Customer as c, Reservation as r, Menu as m where c.Oid = r.customer_id and r.menu_id = m.oid order by r.Oid");
							...
							<tr>
								...
								<td><%=rs.getString("c.oid")%></td>
								<td><%=rs.getString("menuName")%></td>
								<td><%=rs.getString("point")%></td>
								<td><input type="button" value="수정하기"
									onClick="location.href='ModifyReservation.jsp?roid=<%=rs.getString("r.oid")%>&customer_id=<%=rs.getString("c.oid")%>&name=<%=rs.getString("name")%>&phone=<%=rs.getString("phoneNumber")%>&time=<%=rs.getString("time")%>&covers=<%=rs.getString("covers")%>&table_id=<%=rs.getString("table_id")%>'"></td>
								<td><input type="button" value="삭제하기"
									onClick="location.href='Manager_DeleteAction.jsp?roid=<%=rs.getString("r.oid")%>&name=<%=rs.getString("name")%>&phone=<%=rs.getString("phoneNumber")%>'"></td>
							</tr>
							<%
							}
							rs.close();
